...
#列出要检测的系统yum安装的服务名称
services = ['nginx','salt-master','salt-minion','httpd','redis','mariadb']
...
def service_status(ip):
    ...
    for service in services:
        ...
        pkg_installed = client.cmd(ip, 'pkg.info_installed', [service])[ip]
        if isinstance(pkg_installed,dict) is True:
            services_info[service]['installed'] = 'Y'
            pkg_status = client.cmd(ip,'service.show',[service])[ip]['ActiveState']
